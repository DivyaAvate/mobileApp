...
class MainLayout extends StatelessWidget {
  ...
  int _indexFromPath(String path) {
    if (path.startsWith('/workout'))    return 1;
    if (path.startsWith('/progress'))   return 2;
    if (path.startsWith('/coach'))      return 3;
    if (path.startsWith('/profile') ||
        path.startsWith('/achievements') ||
        path.startsWith('/leaderboard')) return 4;
    return 0;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: false, // ← prevent back button exiting to login
      onPopInvokedWithResult: (didPop, result) {
        // If not on home tab, go home instead of exiting
        if (index != 0) {
          context.go('/home');
        }
        // If already on home, do nothing (stay in app)
      },
      child: Scaffold(
